...
%    numIteration,...    number of iterations to perform.%die dai ci shu
%
...
%    (optional, see errorFlag) errorGoal, ...        % allowed representation error in representing each signal.  表示每个信号时允许的表示错误
%    InitializationMethod,...  mehtod to initialize the dictionary, can be one of the following arguments:   初始化字典的方法可以是以下参数之一：
%                                 * 'DataElements' (initialization by the signals themselves), or:   “数据元素”（由信号本身初始化）
%                                 * 'GivenMatrix' (initialization by a given matrix param.initialDictionary).  “GivenMatrix”（通过给定的矩阵参数初始化initialDictionary）。
%    (optional, see InitializationMethod) initialDictionary,...      % if the initialization method   
%                                 is 'GivenMatrix', this is the matrix that will be used.  如果初始化方法为“GivenMatrix”，则将使用该矩阵。
%    (optional) TrueDictionary, ...        % if specified, in each  
%                                 iteration the difference between this dictionary and the trained one  
%                                 is measured and displayed.  如果指定，在每次迭代中，都会测量并显示该词典与经过训练的词典之间的差异。
...
%                                   如果=1，则显示进度信息。如果是param.errorFlag==0，则显示平均重新表述错误（RMSE），如果参数errorFlag==1，显示每个信号表示所需系数的平均数。
% ========================================================================  
% OUTPUT ARGUMENTS:  
%  Dictionary                  The extracted dictionary of size nX(param.K).  提取的字典大小为nX（param.K）。
...
%                                param.errorFlag = 0)  每次迭代后的总表示错误（仅当param.displayProgress=1和param.errorFlag=0时定义）
...
%                                param.displayProgress=1 and  
%                                param.errorFlag = 1)  长度param.numIteration向量，包括表示每个信号（在每次迭代中）所需的系数平均数（仅当param.displayProgress=1和param.errorFlag=1时定义）
% =========================================================================  
